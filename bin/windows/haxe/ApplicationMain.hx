...
class ApplicationMain
{
	...
	public static function create(config):Void
	{
		...
		app.meta["name"] = "TheApp";
		app.meta["packageName"] = "com.example.TheApp";
	...
	macro public static function getEntryPoint()
	{
		...
		switch (Context.follow(Context.getType("TheApp")))
		{
			case TInst(t, params):
				...
				if (hasMain)
				{
					return Context.parse("@:privateAccess TheApp.main()", Context.currentPos());
				}
				...
				{
					Context.fatalError("Main class \"TheApp\" has neither a static main nor a constructor.", Context.currentPos());
				}

			default:

				Context.fatalError("Main class \"TheApp\" isn't a class.", Context.currentPos());
		}
...
@:keep @:dox(hide) class DocumentClass extends TheApp {}
#else
class DocumentClass
{
